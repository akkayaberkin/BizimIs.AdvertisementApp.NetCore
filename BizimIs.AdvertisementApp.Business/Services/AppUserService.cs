...
using BizimIs.AdvertisementApp.Business.Extensions;
...
namespace BizimIs.AdvertisementApp.Business.Services
{
    public class AppUserService : Service<AppUserCreateDto, AppUserUpdateDto, AppUserListDto, AppUser>, IAppUserService
    {
        ...
        public async Task<IResponse<AppUserListDto>> CheckSingUpUsernameAsync(AppUserCreateDto dto)
        {
            var validationResult = _createDtoValidator.Validate(dto);
            if (validationResult.IsValid)
            {
                ...
                return new Response<AppUserListDto>(ResponseType.AlreadyHave, "Kullanıcı adı daha önce kullanılmış.");
            }
            return new Response<AppUserListDto>(ResponseType.ValidationError, "Tüm alanları eksiksiz doldurduğunuzdan emin olun.");
        }
    }
}
