...
using BizimIs.AdvertisementApp.UI.Extensions;
using BizimIs.AdvertisementApp.UI.Models;

namespace BizimIs.AdvertisementApp.UI.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> SignUp(UserCreateModel model)
        {
            ...
            if (result.IsValid)
            {
                var modeldto = _mapper.Map<AppUserCreateDto>(model);

                var check = await _appUserService.CheckSingUpUsernameAsync(modeldto);
                if (check.ResponseType != Common.ResponseType.AlreadyHave)
                {
                ...
                {
                    ModelState.AddModelError("AlreadyHave", "Girmiş olduğunuz kullanıcı adı daha önce alınmış.");
                }
